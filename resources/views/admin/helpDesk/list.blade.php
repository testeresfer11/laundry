              ...
              <div class="col-md-12">
                <ul class="list-group fa-padding">
                  @forelse ($data as $ticket)
                    <li class="list-group-item ticket-list {{ in_array($ticket->id, $newlyUpdatedTicketIds ?? []) ? 'highlight' : '' }}" data-ticket-id="{{$ticket->id}}">
                      <div class="media">
                            ...
                            <span class="number pull-right d-flex">
                              <span class="label label-danger">
                                @switch($ticket->status)
                                    @case('Completed')
                                        <button type="button" class="btn default-btn btn-rounded btn-sm">Completed</button>
                                      @break
                                    @case('In Progress')
                  ...
                  @empty
                      <li class="list-group-item">
                          <strong class="no-record"><div class="col-12 text-center">No record Found</div></strong> 
                      </li>
                  @endforelse
                  <!-- BEGIN DETAIL TICKET -->
...
<script>
  ...
  function highlightTicketItem(ticketId) {
    const item = document.querySelector(`.ticket-list[data-ticket-id="${ticketId}"]`);
    if (item) {
        item.classList.add('ticket-highlight');
        // Remove highlight after a few seconds
        setTimeout(() => {
            item.classList.remove('ticket-highlight');
        }, 3000);
    }
  }

    const newTickets = @json($newlyUpdatedTicketIds ?? []);
    newTickets.forEach(id => highlightTicketItem(id));
